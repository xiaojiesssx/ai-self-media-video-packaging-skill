...
describe('public Skill contract', () => {
  ...
  it('publishes under the current maintainer without personal contact assets', () => {
    expect(readme).toContain('作者与维护者：小杰');
    expect(readme).not.toMatch(new RegExp(['微信', '二维码', ['na', 'na', 'ya093'].join(''), ['na', 'na', '-wechat'].join('')].join('|'), 'i'));
  });
});
