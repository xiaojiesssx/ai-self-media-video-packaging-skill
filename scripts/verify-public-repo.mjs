...
const personalIdentityTerms = [
  new RegExp(['na', 'na'].join(''), 'i'),
  new RegExp(['na', 'na', 'ya093'].join(''), 'i'),
  new RegExp(['zhou', 'yue', 'chuan'].join(''), 'i'),
  new RegExp(['3002', '87844'].join('')),
  new RegExp(['原', '住', '民'].join('')),
];
...
export const scanEntries = (entries) => {
  ...
  for (const entry of entries) {
    ...
    if (personalIdentityTerms.some((pattern) => pattern.test(normalized))) findings.push(`${normalized}: personal-identity-in-path`);
    if (entry.text !== null) {
      ...
      if (personalIdentityTerms.some((pattern) => pattern.test(entry.text))) findings.push(`${normalized}: personal-identity`);
